...
async function initAdminPanel() {
  ...
  function isAuthorized() {
    return (
      window.ADMIN_AUTH && window.ADMIN_AUTH.name && window.ADMIN_AUTH.password
    );
  }
  ...
  previewEl.addEventListener("click", () => {
    ...
    doc.open();
    doc.write(
      "<!DOCTYPE html><html><head><title>Editable JSON</title></head><body></body></html>",
    );
    doc.close();

    const safeJSON = JSON.stringify(jsonText)
      .replace(/\\/g, "\\\\")
      .replace(/`/g, "\\`");
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (
            window.opener &&
            typeof window.opener.loadDatabase === "function"
          ) {
            await window.opener.loadDatabase();
          } else if (
            window.opener &&
            typeof window.opener.refreshDatabase === "function"
          ) {
            await window.opener.refreshDatabase();
          }
        } else {
      ...
      if (
        window.opener &&
        typeof window.opener.loadDatabaseGlobal === "function"
      ) {
        await window.opener.loadDatabaseGlobal();
      }
    };
  });
  ...
  function initAutocomplete() {
    ...
    setupAutocomplete(
      ...
      () => {
        ...
        const uniquePokemon = [
          ...new Map(allPokemon.map((p) => [p.toLowerCase(), p])).values(),
        ];

        return uniquePokemon.map(
  ...
  addBtn.addEventListener("click", async () => {
    ...
    if (modeSelect.value === "pokemon") {
      ...
      const duplicate = last5Ids.some((id) => {
        return (
          playerShinies[id].Pokemon.toLowerCase() === pokemonName.toLowerCase()
        );
      });
      ...
      if (!pokeName || !twitchName) {
        messageEl.textContent =
          "Both PokeMMO Name and Twitch Name are required.";
        messageEl.className = "error";
        return;
      }
